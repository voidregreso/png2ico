...
bool convertToIndexed(png_data &img, bool hasAlpha) {
	...
	while (img.num_palette < maxColors) {
		...
		if (mdqMinDist > 0) { // if we have found a most different quad, add it to the palette
			...
			break; // otherwise (i.e. all quads are mapped) the palette is finished
	};
	...
	while (iter != stop) {
		...
		if ((quad >> 24) != 0) { // if color is not transparent
			...
			temp = (blue - img.palette[i].blue);
			dist += temp * temp;
			if (dist > color_reduce_warning_threshold)
				tooManyColors = true;
		};
	};
	...
	for (int y = img.height - 1; y >= 0; --y) {
		...
		for (unsigned i = 0; i < img.width; ++i) {
			...
			unsigned int quad = pixel[0] + (pixel[1] << 8) + (pixel[2] << 16);
			if (!trans)
				quad += (255 << 24); // NOTE: alpha channel has already been set to 255 for non-transparent pixels, so this is correct even for images with alpha channel
